...
Read `layout` in the resolved YAML first. Those templates are the only
source for where a change, plan, and task board live. Expand `{change}`
and `{task}` from the current run.
...
For medium/large, discover the project's existing homes for decisions,
specifications, and plans. Write there when they exist. Otherwise keep intent,
specification, plan, and state as explicit sections in handoffs; `.work` is
coordination state, not a second documentation system.
...
- Orchestrator: specification/decisions, plan, task boundaries, and state. Medium/large.
...
**Orchestrator:** receive Brain's intent, keep specification and plan separate,
and update coordination state. Use existing project document homes; otherwise
carry those sections in the handoff. Large defines non-overlapping file sets.
Do not spawn agents.

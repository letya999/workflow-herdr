...
class GraphTests(unittest.TestCase):
    def test_default_models_are_unchanged(self) -> None:
        ...
        self.assertEqual(disp["harness"], "codex")
        self.assertEqual(disp["model"], "gpt-5.6-luna")
        self.assertEqual(disp["effort"], "high")
        self.assertEqual(worker["harness"], "devin")
        self.assertEqual(worker["model"], "swe2")
        self.assertEqual(worker["effort"], "max")
        self.assertEqual(native_args(config, "worker", 1, "x"), ["--model", "swe2"])
